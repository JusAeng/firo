...
// Create a transaction to vecSend, placing it in wtxNew. reservekey is the keypool. nFeeRet will be populated with
// the calculated transaction fee. If nChangePosInOut is not -1, the change output will be placed at that position. If
// nChangePosInOut > vecSend.size(), we will fail. If creating a transaction fails, we will return false and set
// strFailReason to a human-readable description of the failure. coinControl may be set or unset; if it is unset
// an arbitrary transaction selection algorithm will be used. If coinControl->nMaxSize is set, transaction creation will
...
bool CWallet::CreateTransaction(const std::vector<CRecipient>& vecSend, CWalletTx& wtxNew, CReserveKey* reservekey,
    ...
    for (CTransparentTxout& txin: vInputTxs) {
        ...
        assert(nNew < txNew.vin.size());
        txNew.vin[nNew].scriptSig = sigData.scriptSig;
        txNew.vin[nNew].scriptWitness = sigData.scriptWitness;
        ...
        if (!Solver(txin.GetScriptPubkey(), whichType, vSolutions))
            strFailReason = _("Non-standard input");
        else if (!success)
            strFailReason = _("Signing transaction failed");
        else if (!sign || ctx.IsCoinBase())
            ;
        else if (sigData.scriptSig.size() > 1560)
            strFailReason = _("Produced a signature which would exceed relay limit");
        else if (!sigData.scriptSig.IsPushOnly())
            strFailReason = _("Produced a signature which is not push only");
        else if (!sigData.scriptSig.HasCanonicalPushes())
            strFailReason = _("Produced a signature which has non-canonical pushes");
        else if (!EvalScript(stack, sigData.scriptSig, SCRIPT_VERIFY_NONE, BaseSignatureChecker(), SIGVERSION_BASE))
            strFailReason = _("Couldn't evaluate produced signature");
        else if (stack.empty())
           strFailReason = _("Produced a signature with an empty sigScript stack");
        else if (whichType == TX_SCRIPTHASH && CScript(stack.back().begin(), stack.back().end()).GetSigOpCount(true) > MAX_P2SH_SIGOPS)
           strFailReason = _("Produced a signature with too many sigops");

        if (!strFailReason.empty()) {
            LogPrintf("%s(): Failed to produce a signature for %s-%d: %s\n", __func__, txin.GetOutpoint().hash.GetHex(),
                      txin.GetOutpoint().n, strFailReason);
            nFeeRet = -1;
            nChangePosInOut = -1;
            return false;
        }
    ...
    int64_t txSize = ::GetSerializeSize(txNew, SER_NETWORK, PROTOCOL_VERSION);
    if (txSize > MAX_STANDARD_TX_WEIGHT) {
        nFeeRet = -1;
        nChangePosInOut = -1;
        strFailReason = _("Transaction too large");
        return false;
    } else if (coinControl && coinControl->nMaxSize && txSize > coinControl->nMaxSize) {
        LogPrintf("We made a transaction exceeding coinControl->nMaxSize. This is a bug.\n");

        nFeeRet = -1;
        nChangePosInOut = -1;
        strFailReason = _("We made a transaction exceeding coinControl->nMaxSize. This is a bug.");
        return false;
    }

    if (coinControl && coinControl->nMaxInputs && txNew.vin.size() > coinControl->nMaxInputs) {
        LogPrintf("We made a transaction exceeding coinControl->nMaxInputs. This is a bug.\n");

        nFeeRet = -1;
        nChangePosInOut = -1;
        strFailReason = _("We made a transaction exceeding coinControl->nMaxInputs. This is a bug.");
        return false;
    }

    if (nFeeRet < GetFee(coinControl, txSize)) {
        LogPrintf("Calculated fee too low. This is a bug.");

        nFeeRet = -1;
        nChangePosInOut = -1;
        strFailReason = _("Calculated fee too low. This is a bug.");
        return false;
    }
    ...
    size_t maxSize = txSize + vInputTxs.size() * 3;
    // If there is no change output, our fee may have been calculated based on dust being offered to miners.
    if (nChangePosInOut == -1) maxSize += P2SH_OUTPUT_SIZE;
    if (sign && nFeeRet > GetFee(coinControl, maxSize)) {
        LogPrintf("Calculated fee too high. This is probably a bug.");

        nFeeRet = -1;
        nChangePosInOut = -1;
        strFailReason = _("Calculated fee too high. This is probably a bug.");
        return false;
    }

    if (GetRequiredFee(txSize) > nFeeRet) {
        nFeeRet = -1;
        nChangePosInOut = -1;
        strFailReason = _("Calculated transaction fee below relay fee");
        return false;
    }
    ...
    if (nFeeRet > maxTxFee) {
        nFeeRet = -1;
        nChangePosInOut = -1;
        strFailReason = _("txFee > maxTxFee; this is probably an error");
        return false;
    }
    ...
    for (CTxOut& txout: txNew.vout) {
        ...
        if (!::IsStandard(txout.scriptPubKey, whichType, false)) {
            LogPrintf("%s(): Aborting due to having created a non-standard txout.\n", __func__);
            nFeeRet = -1;
            nChangePosInOut = -1;
            strFailReason = _("Created a non-standard txout");
            return false;
        }

        if (whichType == TX_NULL_DATA) {
            if (fHasDataOut) {
                LogPrintf("%s(): Aborting due to having created a too many TX_NULL_DATA outputs.\n", __func__);
                nFeeRet = -1;
                nChangePosInOut = -1;
                strFailReason = _("Created multiple TX_NULL_DATA outputs");
                return false;
            }
            fHasDataOut = true;
        } else if ((whichType == TX_MULTISIG) && (!fIsBareMultisigStd)) {
            LogPrintf("%s(): Aborting due to having created a non-standard bare multisig output\n", __func__);
            nFeeRet = -1;
            nChangePosInOut = -1;
            strFailReason = _("Created non-standard bare multisig output");
            return false;
        }

        if (txout.IsDust()) {
            LogPrintf("%s(): Aborting due to having created a dust outputs.\n", __func__);
            nFeeRet = -1;
            nChangePosInOut = -1;
            strFailReason = _("Created a dust output");
            return false;
        }
    }
    ...
    for (CTransparentTxout& txin: vInputTxs) totalInputValue += txin.GetValue();
    for (CTxOut& txout: txNew.vout) totalOutputValue += txout.nValue;
    assert(totalInputValue == totalOutputValue + nFeeRet);
