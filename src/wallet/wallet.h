...
private:
    ...
    void GetInputsForTx(const std::vector<AbstractTxout>& vRelevantTransactions, std::vector<AbstractTxout>& vInputs,
        ...
        for (std::vector<AbstractTxout>* v: {&vAvailable, &vCoinControlInputs}) {
        ...
        // coinControl->fRequireAllInputs is not set, it will stop when it has enough to provide for our outputs, otherwise
        // it will consume all the inputs. After that, it will select the smallest UTXO in our wallet, and end if enough
        // value is found. Then, if there is a UTXO which combined with the smallest that can provide for our entire output
        // value, that UTXO. If there is not, it will then pick the largest UTXO, and then the next smallest, and so on and
        // so forth until we can fulfill our output requirements, or until we reach nMaxSize or coinControl->nMaxInputs
        // limits. Once we reach those, we will start replacing the largest small inputs we have chosen with the largest
        // inputs we haven't selected yet. If that fails, it is the case that the transactions in our wallet are not
        // sufficient to provide for the outputs requested within the coinControl and nMaxSize constraints, so we will
        // therefore fail.
        ...
        while (true) {
            // The idea here is to front-side inputs which are larger than the smallest front-side input that can fulfill
            // our output value requirements.
            if (fTrySkipFront && iCoinControl == vCoinControlInputs.size()) {
                if (iFront) {
                    ...
                    // If we're at the first iteration, we'll identify the smallest input that's larger than nRequired and
                    // start looking for inputs from that.
                    ...
                    // previously, we don't have any UTXOs which will fulfill our requirements. Therefore, we'll stop using
                    // the fTrySkipFront logic and pick the largest UTXOs we have.
                ...
                // If we've done an iteration of fTrySkipFront logic before, fTakeFromFront will be false, but we need it
                // to be true as this is a do-over.
                fTakeFromFront = true;
            }
            ...
            try {
                ...
                // If we push this to the back of vSmallInputs, it will be replaced in the next iteration as the condition
                // of the first if statement at the beginning of this loop will be fulfilled. nCollectedRet has been mutated
                // above so that the undo in the first if block will be valid, and txSize will not be modified in the event
                // we can't get the input size for this txo.
                if (hasReplacedTxout) vSmallInputs.emplace_back(iToReplace);

                continue;
            }

            // The number of inputs is encoded as a varint, so if there are more than 0xfd inputs we need to add 2 bytes.
            // If we're replacing inputs with fTrySkipFront logic, GetSizeOfCompactSize will return 1 for both values, so
            // no special logic to handle that case is required.
            size_t varintSizeDifference = hasReplacedTxout ? 0 :
                                          GetSizeOfCompactSize(vInputs.size() + 1) -
                                          GetSizeOfCompactSize(vInputs.size());

            // If this transaction will bring us over nMaxSize, don't add it and start transaction replacement logic above.
            // It is technically possible for our transaction finding logic to fail to find a possible solution in the event
            // that a lower value transaction has a smaller spend script, and the difference in fees between the two exceeds
            // the difference in value, and this is true for every transaction larger than the transaction with the smaller
            // spend script, and the difference is over what we need to get to nRequired, but this case should be extremely
            // unlikely.
            ...
            // If coin control is enabled, we want to select all inputs, so we will only check whether we have enough after
            // exhausting vAvailable.
